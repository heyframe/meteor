 ...
 */
import type Repository from "@heyframe-ag/meteor-admin-sdk/es/data/repository";
import Criteria from "@heyframe-ag/meteor-admin-sdk/es/data/Criteria";
import { useRepository } from "@heyframe-ag/meteor-admin-sdk/es/data/composables/useRepository";
import { getRepository } from "@heyframe-ag/meteor-admin-sdk/es/data/composables/getRepository";
import MtDataTable, {
  type ColumnChanges,
...
const handleItemDelete = async ({ id }: { id: string }) => {
  ...
  if (isInIframe && !props.forceRealModal) {
    const { ui } = await import("@heyframe-ag/meteor-admin-sdk");
