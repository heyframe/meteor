...
describe("mt-url-field", () => {
  ...
  it("updates the domain when the user types", async () => {
    ...
    // ACT
    await userEvent.type(screen.getByRole("textbox"), "www.heyframe.com");

    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("www.heyframe.com");

    expect(handler).toHaveBeenCalledTimes(16);
    expect(handler).toHaveBeenNthCalledWith(1, "https://w");
    expect(handler).toHaveBeenNthCalledWith(16, "https://www.heyframe.com");
  });
  ...
  it("updates the domain when the user types and then focuses another element", async () => {
    ...
    expect(handler).toHaveBeenCalledWith("www.heyframe.com");
  });
