...
/**
 * Resembles the options that are available on any HeyFrameMessageType.
 */
 ...
 * @internal
 */
export type MessageDataType<TYPE extends keyof HeyFrameMessageTypes> = Omit<HeyFrameMessageTypes[TYPE], 'responseType'>;

/**
 * This is the structure of the data which will be send with {@link send}
 * @internal
 */
export type HeyFrameMessageSendData<MESSAGE_TYPE extends keyof HeyFrameMessageTypes> = {
  _type: MESSAGE_TYPE,
 ...
 * @internal
 */
export type HeyFrameMessageResponseData<MESSAGE_TYPE extends keyof HeyFrameMessageTypes> = {
  _type: MESSAGE_TYPE,
  _response: HeyFrameMessageTypes[MESSAGE_TYPE]['responseType'] | null,
  _callbackId: string,
}
 ...
 */
export function send<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(
  type: MESSAGE_TYPE,
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  _targetWindow?: Window,
  _origin?: string
): Promise<HeyFrameMessageTypes[MESSAGE_TYPE]['responseType'] | null> {
  ...
  const messageData: HeyFrameMessageSendData<MESSAGE_TYPE> = {
    _type: type,
    _data: sendData,
  ...
  let serializedData = serialize(messageData) as HeyFrameMessageSendData<MESSAGE_TYPE>;
  ...
  if (_origin) {
    ...
    if (validationErrors) {
      ...
      if ([
        ...
        }) as HeyFrameMessageSendData<MESSAGE_TYPE>;
      }
  ...
  return new Promise((resolve, reject) => {
    const callbackHandler = function(event: MessageEvent<string>):void {
      ...
      let heyframeResponseData;
      // Try to parse the json response
      try {
        heyframeResponseData = JSON.parse(event.data) as unknown;
      } catch {
      ...
      if (!isMessageResponseData<MESSAGE_TYPE>(heyframeResponseData)) {
        return;
      }

      // Only execute if response value exists
      if (!heyframeResponseData.hasOwnProperty('_response')) {
        return;
      }

      // Deserialize methods etc. so that they are callable in JS
      const deserializedResponseData = deserialize(heyframeResponseData, event) as HeyFrameMessageResponseData<MESSAGE_TYPE>;
...
export type HandleMethod<MESSAGE_TYPE extends keyof HeyFrameMessageTypes> = (
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  additionalInformation: { _event_: MessageEvent<string> }
) => Promise<HeyFrameMessageTypes[MESSAGE_TYPE]['responseType']> | HeyFrameMessageTypes[MESSAGE_TYPE]['responseType'];
 ...
 */
export function handle<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>
(
  type: MESSAGE_TYPE,
  ...
  : () => void
{
  const handleListener = async function(event: MessageEvent<string>): Promise<void> {
    ...
    try {
      heyframeMessageData = JSON.parse(event.data) as unknown;
    } catch {
      // Fail silently when message is not a valid json file
      return;
    }
    ...
    // Check if messageData is valid
    if (!isMessageData<MESSAGE_TYPE>(heyframeMessageData)) {
      return;
    }

    // Deserialize methods etc. so that they are callable in JS
    const deserializedMessageData = deserialize(heyframeMessageData, event) as HeyFrameMessageSendData<MESSAGE_TYPE>;
    ...
    const responseMessage: HeyFrameMessageResponseData<MESSAGE_TYPE> = {
    ...
    const serializedResponseMessage = ((): HeyFrameMessageResponseData<MESSAGE_TYPE> => {
      let serializedMessage = serialize(responseMessage) as HeyFrameMessageResponseData<MESSAGE_TYPE>;
      ...
      if (validationErrors) {
        ...
        serializedMessage = serialize(serializedMessage) as HeyFrameMessageResponseData<MESSAGE_TYPE>;
      }
...
export function publish<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(
  type: MESSAGE_TYPE,
  data: HeyFrameMessageTypes[MESSAGE_TYPE]['responseType'],
  sources: {
    source: Window,
    origin: string,
    sdkVersion: string | undefined,
...
export function subscribe<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(
  type: MESSAGE_TYPE,
  method: (data: HeyFrameMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>
): () => void {
...
export function createSender<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>
(messageType: MESSAGE_TYPE, baseMessageOptions: MessageDataType<MESSAGE_TYPE>)
:(messageOptions?: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions) => Promise<HeyFrameMessageTypes[MESSAGE_TYPE]['responseType']>

// SENDER WITH PARTIAL ARGUMENTS (ARGUMENTS DEFINED IN BASE OPTIONS ARE OMITTED)
export function createSender<MESSAGE_TYPE extends keyof HeyFrameMessageTypes, BASE_OPTIONS extends Partial<MessageDataType<MESSAGE_TYPE>>>
(messageType: MESSAGE_TYPE, baseMessageOptions: BASE_OPTIONS)
:(messageOptions: Omit<MessageDataType<MESSAGE_TYPE>, keyof BASE_OPTIONS> & BaseMessageOptions) => Promise<HeyFrameMessageTypes[MESSAGE_TYPE]['responseType']>
 ...
 */
export function createHandler<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
 ...
 * defined and can be hidden.
 */
export function createSubscriber<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
  return (method: (data: HeyFrameMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>, id?: string) => {
    if (!id) {
      return subscribe(messageType, method);
    }

    const wrapper = (data: HeyFrameMessageTypes[MESSAGE_TYPE]['responseType']): void => {
      if (data.id === id) {
        void method(data);
      }
    };
 ...
 */
function isMessageData<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(eventData: unknown): eventData is HeyFrameMessageSendData<MESSAGE_TYPE> {
  const heyframeMessageData = eventData as HeyFrameMessageSendData<MESSAGE_TYPE>;
...
function isMessageResponseData<MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(eventData: unknown): eventData is HeyFrameMessageResponseData<MESSAGE_TYPE> {
  const heyframeMessageData = eventData as HeyFrameMessageResponseData<MESSAGE_TYPE>;
