import getCompareIsHeyFrameVersion from './compare-version';

type Comparator = '=' | '!=' | '<' | '>' | '<=' | '>=';

describe('getCompareHeyFrameVersion', () => {
  const mock = (version: string) => () => Promise.resolve(version);

  describe('It works with heyframe versions and semver versions', () => {
    it.each<[boolean, string, Comparator, string]>([
      ...
      [false, '6.8.2', '=', '6.6.9.2'],
      [false, '6.8.2', '=', '6.9.2'],
    ])("returns %s for '%s %s %s'", async (exptectedResult, heyframeVersion, comparator, comparedVersion) => {
      const compareIsHeyFrameVersion = getCompareIsHeyFrameVersion(mock(heyframeVersion));

      expect(await compareIsHeyFrameVersion(comparator, comparedVersion)).toBe(exptectedResult)
    });
  });
