<template>
  <SwCard title="Context">
    ...
    <div class="context-functionality-buttons">
      ...
      <SwButton @click="getCurrentHeyFrameVersion">
...
import { context, notification } from '@heyframe-ag/meteor-admin-sdk';
import { SwCard, SwButton } from '@heyframe-ag/meteor-component-library';
...
async function getCurrentHeyFrameVersion() {
  const currentHeyFrameVersion = await context.getHeyFrameVersion();

  notification.dispatch({
    title: 'Current HeyFrame version',
    message: `
    Your current HeyFrame version is "${currentHeyFrameVersion}".
    `
  })
}
