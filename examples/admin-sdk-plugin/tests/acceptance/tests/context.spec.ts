...
test("@sdk: subscribe on locale changes", async ({
  ...
  await ShopAdmin.page.locator("#sw-field--confirm-password").fill("heyframe");
...
test("@sdk: get current heyframe version", async ({ ShopAdmin }) => {
  await ShopAdmin.page.locator(".sw-order").click();
  ...
  await frame
    .locator("button", { hasText: "Get current HeyFrame version" })
    .click();

  await expect(
    frame.locator("p", { hasText: "HeyFrame version: 6." }),
  ).toBeVisible();
});
